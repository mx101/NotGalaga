...
void ofxXboxController::update() {
    ...
    for (DWORD i = 0; i < MAX_CONTROLLERS; i++) {
        ...
        if (gamepads[i].connected) {
            WORD wButtons = g_Controllers[i].state.Gamepad.wButtons;
            gamepads[i].dPadUp =
                (wButtons & XINPUT_GAMEPAD_DPAD_UP) ? true : false;
            gamepads[i].dPadDown =
                (wButtons & XINPUT_GAMEPAD_DPAD_DOWN) ? true : false;
            gamepads[i].dPadLeft =
                (wButtons & XINPUT_GAMEPAD_DPAD_LEFT) ? true : false;
            gamepads[i].dPadRight =
                (wButtons & XINPUT_GAMEPAD_DPAD_RIGHT) ? true : false;
            gamepads[i].start =
                (wButtons & XINPUT_GAMEPAD_START) ? true : false;
            gamepads[i].back = (wButtons & XINPUT_GAMEPAD_BACK) ? true : false;
            gamepads[i].leftThumb =
                (wButtons & XINPUT_GAMEPAD_LEFT_THUMB) ? true : false;
            gamepads[i].rightThumb =
                (wButtons & XINPUT_GAMEPAD_RIGHT_THUMB) ? true : false;
            gamepads[i].leftShoulder =
                (wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) ? true : false;
            gamepads[i].rightShoulder =
                (wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER) ? true : false;
            ...
            gamepads[i].leftTrigger = ofMap(
                g_Controllers[i].state.Gamepad.bLeftTrigger, 0, 255, 0, 1);
            gamepads[i].rightTrigger = ofMap(
                g_Controllers[i].state.Gamepad.bRightTrigger, 0, 255, 0, 1);
            gamepads[i].thumbLX = ofMap(g_Controllers[i].state.Gamepad.sThumbLX,
                                        -32767, 32767, -1, 1);
            gamepads[i].thumbLY = ofMap(g_Controllers[i].state.Gamepad.sThumbLY,
                                        -32767, 32767, -1, 1);
            gamepads[i].thumbRX = ofMap(g_Controllers[i].state.Gamepad.sThumbRX,
                                        -32767, 32767, -1, 1);
            gamepads[i].thumbRY = ofMap(g_Controllers[i].state.Gamepad.sThumbRY,
                                        -32767, 32767, -1, 1);
            ...
            g_Controllers[i].vibration.wLeftMotorSpeed =
                (WORD)ofMap(gamepads[i].leftVibration, 0, 1, 0, 65535);
            g_Controllers[i].vibration.wRightMotorSpeed =
                (WORD)ofMap(gamepads[i].rightVibration, 0, 1, 0, 65535);
...
void ofxXboxController::draw() {
    ...
    ofDrawBitmapString(
        "The state of all 4 XInput controllers\nPress 'd' to toggle dead zone "
        "clamping.",
        20, 20);
    ...
    for (DWORD i = 0; i < MAX_CONTROLLERS; i++) {
        ...
        ofDrawBitmapString("Controller " + ofToString(i) + ": " + cval, 20,
                           15 * i + 60);
        ...
        if (gamepads[i].connected) {
            swprintf_s(
                ...
                (wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) ? L"LEFT_SHOULDER "
                                                          : L"",
                (wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER) ? L"RIGHT_SHOULDER "
                                                           : L"",
                ...
                gamepads[i].leftTrigger, gamepads[i].rightTrigger,
                gamepads[i].thumbLX, gamepads[i].thumbLY, gamepads[i].thumbRX,
                gamepads[i].thumbRY);
